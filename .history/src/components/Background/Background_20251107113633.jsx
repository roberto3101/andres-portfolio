...
function StarField({ isMobile }) {
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Colores variados entre azules
      const intensity = Math.random();
      colors[i * 3] = 0.3 + intensity * 0.2;
      colors[i * 3 + 1] = 0.6 + intensity * 0.2;
  ...
  return (
    <div className={styles.background}>
      ...
      >
        ...
        <fog attach="fog" args={['#0a0814', 15, 45]} />
        
        <ambientLight intensity={0.3} />
        <pointLight position={[0, 0, 0]} intensity={1.2} color="#50c9ff" distance={35} />
        <pointLight position={[10, 10, 10]} intensity={0.6} color="#4a90e2" />
        <pointLight position={[-10, -10, -5]} intensity={0.4} color="#7ab8f5" />
